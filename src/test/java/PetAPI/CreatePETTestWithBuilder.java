...
public class CreatePETTestWithBuilder {
    ...
    @Test
    public void createAPetWithBuilderTest(){
        //post
        //serialization- pojo to json

        PetLombok.Category category= new PetLombok.Category.CategoryBuilder().id(1).name("Dog").build();
        PetLombok.Tags tag1=new PetLombok.Tags.TagsBuilder().id(1).name("Red").build();
        PetLombok.Tags tag2=new PetLombok.Tags.TagsBuilder().id(2).name("Black").build();
        List<PetLombok.Tags> tags=Arrays.asList(tag1, tag2);

        List<String> photoUrls= Arrays.asList("https://www.abc.com", "https://www.xyz.com");

        PetLombok petData= new PetLombok.PetLombokBuilder().id(101).name("tommy").status("available")
                .category(category).photoUrls(photoUrls).tags(tags).build();
